...
// récupération des informations passée en _GET pour cette partie du code (affichage de la liste + détail d'une livraison)
...
switch($get_action){
    // dans ce cas-ci, on désire générer la liste des livraisons ordonnée par ordre alphabétique et filtré par initiale
    case "liste":
    ...
    // dans ce cas-ci, on désire ajouter une livraison
    // deux cas de figure :
    ...
    case "add":
        ...
        $input[] = addLayout("<h4>Ajout d'une livraison</h4>");
        $input[] = addLayout("<div class='row'>");
        $input[] = addInput('Mode de livraison', ["type" => "text", "value" => $post_delivery, "name" => "delivery", "class" => "u-full-width"], true, "twelwe columns");
        $input[] = addLayout("</div>");
        ...
        if($show_form != false){
            ...
            // on demande à afficher la liste des livraisons
            $page_view = "delivery_liste";
    ...
    case "update":
        ...
        $input = [];
        // ajout des différents champs du formulaire
        $input[] = addLayout("<h4>Modification d'une livraison</h4>");
        $input[] = addLayout("<div class='row'>");
        $input[] = addInput('Mode de livraison', ["type" => "text", "value" => $post_delivery, "name" => "delivery", "class" => "u-full-width"], true, "twelwe columns");
        $input[] = addLayout("</div>");
    ...
    case "showHide":
        ...
        if(showHideDelivery($get_delivery_id)){
            // message de succes qui sera affiché dans le <body>
            $msg = "<p>Mise à jour de la livraison réalisée avec succès</p>";
            $msg_class = "success";
        }else{
            // message d'erreur qui sera affiché dans le <body>
            $msg = "<p>Erreur lors de la mise à jour de la livraison</p>";
            $msg_class = "error";
        }
